...
class BollyBottle3D {
  ...
  init() {
    ...
    // 2. Camera Setup
    // Adjusted FOV and distance to keep the bottle sized perfectly in its container
  ...
  buildBottle() {
    ...
    // We create a cylinder for the body.
    // Height: 1.8, Radius: 0.54, 64 segments for circular perfection.
    ...
    // Created using a Torus and a Sphere section or by styling the cap
    ...
    // Bottle neck (sits on top of the shoulder)
    ...
    // Pump collar (the wider screw-on portion)
    ...
    // Pump stem (the thin vertical straw mechanism)
    ...
    // Pump dispenser head (the actual press cap)
    ...
    // Pump spout / nozzle (protruding arm)
    // Using a custom shape or standard box representation
    const nozzleGeo = new THREE.BoxGeometry(0.12, 0.1, 0.44);
    const nozzleMesh = new THREE.Mesh(nozzleGeo, whitePlasticMaterial);
    nozzleMesh.position.set(0, 1.62, 0.22); // Extends forward along Z-axis
    // Tilt the spout slightly downwards for premium design feel
    ...
    const labelRadius = 0.545; // Sits slightly outside the body to prevent z-fighting
    const labelGeo = new THREE.CylinderGeometry(labelRadius, labelRadius, labelHeight, 64, 1, true); // Open ended cylinder
    ...
    // Align label texture facing front of camera
    ...
    const shadowMaterial = new THREE.MeshBasicMaterial({
      map: shadowTexture,
      transparent: true,
      depthWrite: false, // Prevents z-order artifacts
      opacity: 0.55
    });

    const shadowMesh = new THREE.Mesh(shadowGeo, shadowMaterial);
    shadowMesh.rotation.x = -Math.PI / 2;
    shadowMesh.position.y = -1.1; // Sits directly below the base
  ...
  createLabelTexture() {
    ...
    // Draw grid styling elements or side details (optional premium flair)
    ...
    // Use bold italic serif/sans-serif styling for "bolly" to match reference design
  ...
  createShadowTexture() {
    ...
    // Draw soft radial black gradient
  ...
  setupEvents() {
    ...
    el.addEventListener('pointerdown', (e) => {
      // Capture pointer id for multi-touch handling stability
  ...
  animate() {
    ...
    if (!this.isDragging && timeSinceInteraction > this.idleDelay) {
      ...
      // Gently return X-tilt to default
